...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $idArtigo = !empty($_POST['idArtigo']) ? $_POST['idArtigo'] : "NULL";
  $texto = $_POST['textoSugestao'];
  $estado = $_POST['estadoSugestao'];

  $sql = "INSERT INTO tb_sugestao 
            (idUtilizador, idArtigo, textoSugestao, estadoSugestao)
          VALUES 
            ('$idUtilizador', $idArtigo, '$texto', '$estado')";

  if ($ligacao->query($sql)) {
    header("Location: sugestoesListar.php");
    exit;
  } else {
    $mensagem = "Erro ao inserir a sugestão.";
  }
...
<head>
  ...
  <title>Inserir sugestão | Cardápio</title>
...
<section class="card shadow-sm rounded-4">
  <div class="card-body p-5 text-center">
    ...
    <h1 class="h3 mb-3">Inserir sugestão</h1>

    <?php if ($mensagem): ?>
      <div class="alert alert-danger"><?= $mensagem ?></div>
    <?php endif; ?>

    <a href="sugestoesListar.php" class="btn btn-primary mt-3">Voltar à lista</a>
