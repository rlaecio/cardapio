...
$sql = "SELECT 
          ...
          s.estadoSugestao
        FROM tb_sugestao s
        INNER JOIN tb_utilizador u ON s.idUtilizador = u.idUtilizador
        LEFT JOIN tb_artigo a ON s.idArtigo = a.idArtigo";
...
<head>
  ...
  <title>Listagem de sugestões | Cardápio</title>
...
<section class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
  <div>
    <h1 class="h2 mb-1">Listagem de sugestões</h1>
    <p class="text-secondary mb-0">Página base para listar sugestões dos utilizadores.</p>
  </div>
  <div class="d-flex gap-2">
    <a href="sugestoesCriar.php" class="btn btn-primary">Novo registo</a>
      ...
      <table class="table table-striped table-hover align-middle">
        ...
        <?php if ($result->num_rows > 0): ?>
          <?php while($row = $result->fetch_assoc()): ?>
            <tr>
              <td><?= $row['idSugestao'] ?></td>
              <td><?= $row['nomeUtilizador'] ?></td>
              <td><?= $row['designacaoArtigo'] ?? '—' ?></td>
              <td><?= $row['textoSugestao'] ?></td>
              <td><?= $row['estadoSugestao'] ?></td>
              <td class="text-center">
                <a href="sugestoesEditar.php?id=<?= $row['idSugestao'] ?>" class="btn btn-sm btn-outline-primary me-2">Editar</a>
                <a href="sugestoesEliminar.php?id=<?= $row['idSugestao'] ?>" class="btn btn-sm btn-outline-danger">Eliminar</a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
