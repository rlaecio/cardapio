...
if (isset($_GET['id'])) {
  ...
  $sql = "DELETE FROM tb_sugestao WHERE idSugestao = $id";

  if ($ligacao->query($sql)) {
    header("Location: sugestoesListar.php");
    exit;
  } else {
    $mensagem = "Erro ao eliminar a sugestão.";
  }

} else {
  $mensagem = "ID da sugestão não fornecido.";
}
?>
...
<head>
  ...
  <title>Eliminar sugestão | Cardápio</title>
...
<section class="card shadow-sm rounded-4">
  <div class="card-body p-5 text-center">
    ...
    <h1 class="h3 mb-3">Eliminar sugestão</h1>
