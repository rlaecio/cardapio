...
if (isset($_GET['id'])) {
  ...
  $sql = "DELETE FROM tb_utilizador WHERE idUtilizador = $id";

  if ($ligacao->query($sql)) {
    header("Location: utilizadoresListar.php");
    exit;
  } else {
    $mensagem = "Erro ao eliminar o utilizador. Pode ter sugestões associadas.";
  }

} else {
  $mensagem = "ID do utilizador não fornecido.";
}
?>
...
<head>
  ...
  <title>Eliminar utilizador | Cardápio</title>
...
<section class="card shadow-sm rounded-4">
  <div class="card-body p-5 text-center">
    ...
    <h1 class="h3 mb-3">Eliminar utilizador</h1>
