...
<head>
  ...
  <title>Listagem de utilizadores | Cardápio</title>
...
<section class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
  <div>
    <h1 class="h2 mb-1">Listagem de utilizadores</h1>
    <p class="text-secondary mb-0">Página base para listar utilizadores registados.</p>
  </div>
  <div class="d-flex gap-2">
    <a href="utilizadoresCriar.php" class="btn btn-primary">Novo registo</a>
        ...
        <?php if ($result->num_rows > 0): ?>
          <?php while($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= $row['emailUtilizador'] ?></td>
              <td><?= $row['estadoUtilizador'] ?></td>
              <td class="text-center">
                <a href="utilizadoresEditar.php?id=<?= $row['idUtilizador'] ?>" class="btn btn-sm btn-outline-primary me-2">Editar</a>
                <a href="utilizadoresEliminar.php?id=<?= $row['idUtilizador'] ?>" class="btn btn-sm btn-outline-danger">Eliminar</a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
